...
Reuse an existing layout. New projects may use:
...
Generate detailed technical prose when the plan supplies concrete content or a task needs it. Avoid speculative placeholder documents. Never generate completed implementation logs or passing evidence for future code.

## Task execution

A bundle contains ID; current model/source snapshot; objective; exclusions; requirement and acceptance IDs; dependencies and state; decisions; exact files/symbols; global invariants; relevant contracts; output scope; verification commands or pending analyzer needs; required evidence; stop conditions.

A ready task has sufficient inputs, satisfied dependencies, accepted required decisions, and observable criteria. A lifecycle can be `todo -> ready -> in_progress -> verifying -> done`, with `blocked` and `needs_revalidation` retaining reasons and history.
...
2. Classify intent, implementation, state, and generated-content changes. Preserve IDs and link replacements.
